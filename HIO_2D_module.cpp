...
{   
    ...
    srand (time(NULL) + myid*10);  
    
	// construct a set of Hermite-symmetrized random phase  
    for (i=0;i<n1;i++){
    	for (j=0;j<n2;j++){
        	out[j+n1*i][0] = rand();
            out[j+n1*i][1] = 0;
	...
	// Normalze "in" and assign the intensity from HIOinput to "in".
    double norm=0;
    for (i=0;i<n1;i++){
    	for (j=0;j<n2;j++){
        	norm = sqrt( pow(in[j+n1*i][0],2) + pow(in[j+n1*i][1],2) );
            if (norm<epsilon){
            	 cout<<"Norm is too small, and it might cause zero-division error."<<endl;
				 exit(0);
		    }
            else{
